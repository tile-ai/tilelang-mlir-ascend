...
class InputShapeInference:
                ...
                for region in regions:
                    ...
                    if name in shape:  # simply merge two bounds
                        bounds = [
                            ...
                            for x, y in zip(shape[name], bounds, strict=False)
                        ]
                    shape[name] = bounds
    ...
    def get_input_exprs(self, output_exprs):
        ...
        for dep in reversed(self.deps):
            for var, expr in zip(
                dep.var_map.values(), result[dep.output], strict=False
            ):
                ana.bind(var, expr)
