...
def sparse_attn_kernel(block_top_k, block_heads, num_heads, dim, scale=None, dtype=BF16,
                       accum_dtype=FP32, indices_dtype=INT32):
    if scale is None:
        scale = (1.0 / dim) ** 0.5
    else:
        scale = scale
    ...
    def sparseAttn(
        ...
        with T.Kernel(batch_size * seq_len, is_npu=True) as (cid, _):
            ...
            acc_s_cast = T.alloc_shared((block_heads, block_top_k), dtype)
            ...
            scores_max_prev = T.alloc_fragment((block_heads, 1), accum_dtype)
            scores_scale = T.alloc_fragment((block_heads, 1), accum_dtype)
            scores_sum = T.alloc_fragment((block_heads, 1), accum_dtype)
            sum_exp = T.alloc_fragment((block_heads, 1), accum_dtype)
            valid_mask = T.alloc_fragment((1, block_top_k), accum_dtype)

            for n in T.Pipelined(T.ceildiv(num_heads, block_heads), num_stages=2):
                T.vbrc(value_zero, acc_o)
                T.vbrc(value_zero, sum_exp)
                T.vbrc(-T.infinity(FP32), scores_max)
                T.copy(Q[by, bx, n * block_heads, 0], q_shared)
                for k in T.Pipelined(T.ceildiv(top_k, block_top_k), num_stages=2):
                    real_block_top_k = T.min(top_k - k * block_top_k, block_top_k)
                    if n == 0:
                        T.copy(TopKIndices[by, bx, k * block_top_k], idxs)
                        T.vbrc(value_zero, kv_shared)
                        T.vbrc(value_zero, valid_mask)
                        for i in T.serial(real_block_top_k):
                            cur_idx = idxs[i]
                            if cur_idx != -1:
                                valid_mask[0, i] = 1.
                                T.copy(KV[by, cur_idx, 0], kv_shared[i, 0], size=[1, dim])
                        T.copy(valid_mask, ValidMaskBuffer[by, bx, k * block_top_k])
                        T.copy(kv_shared, SparseKVBuffer[by, bx, k * block_top_k, 0])
                    T.copy(SparseKVBuffer[by, bx, k * block_top_k, 0], kv_shared)

                    T.gemm(q_shared, kv_shared, acc_s, initC=True, b_transpose=True)
                    for i, j in T.Parallel(block_heads, block_top_k):
                        acc_s[i, j] *= scale

                    T.vadd(scores_max, value_zero, scores_max_prev)
                    ...
                    T.copy(ValidMaskBuffer[by, bx, k * block_top_k], valid_mask)
                    for i, j in T.Parallel(block_heads, block_top_k):
                    ...
                    for i, j in T.Parallel(block_heads, dim):
                        acc_o[i, j] *= scores_scale[i, 0]
                    T.gemm(acc_s_cast, kv_shared, acc_o, initC=False)

                T.copy(AttnSink[n * block_heads, 0], attn_sink_shared)
                for i in T.Parallel(block_heads):
                    sum_exp[i, 0] += T.exp(attn_sink_shared[i, 0] - scores_max[i, 0])
                for i, j in T.Parallel(block_heads, dim):
                    acc_o[i, j] /= sum_exp[i, 0]
...
def next_divisible_number(num, divisor):
    return num + divisor - (num % divisor) if num % divisor != 0 else num + divisor
...
def sparse_attn(
    ...
    if not hasattr(sparse_attn, 'kernel') or sparse_attn.num_heads != num_heads or sparse_attn.dim != dim:
        ...
        bytes_workspace = block * dim * 2 + block_heads * block * 4 * 2 + block_heads * dim * 4
        os.environ['TILELANG_ASCEND_WORKSPACE_SIZE'] = str(bytes_workspace * 16)
        sparse_attn.kernel = sparse_attn_kernel(block, block_heads, num_heads, dim, softmax_scale)
        sparse_attn.num_heads = num_heads
        sparse_attn.dim = dim
    output = torch.empty_like(q, device=q.device)
    sparse_kv_buffer = torch.empty((batch_size, seq_len, next_divisible_number(top_k, block), dim),
                                   dtype=q.dtype, device=q.device)
    valid_mask_buffer = torch.empty((batch_size, seq_len, next_divisible_number(top_k, block)),
                                    dtype=attn_sink.dtype, device=attn_sink.device)

    sparse_attn.kernel(q, kv.contiguous(), output, attn_sink, topk_idxs, sparse_kv_buffer, valid_mask_buffer)
    return output
...
def sparse_attn_torch(q: torch.Tensor, kv: torch.Tensor, attn_sink: torch.Tensor, topk_idxs: torch.Tensor,
    ...
    ref_output = softmax_with_sink(
        ...
        dim=-1).to(base_dtype) @ kv_sparse

    return ref_output


def rand_sparse_attn_input(batch_size, num_heads, seq_len, seq_len_kv, top_k, dim, seed=88888888):
