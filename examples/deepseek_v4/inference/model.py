...
class ParallelEmbedding(nn.Module):
    ...
    def __init__(self, vocab_size: int, dim: int):
        ...
        self.weight = nn.Parameter(torch.rand(self.part_vocab_size, self.dim))
...
def linear(x: torch.Tensor, weight: torch.Tensor, bias: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    if weight.dtype == torch.float8_e4m3fn:
        x, s = act_quant(x, block_size, scale_fmt, scale_dtype, backend="torch")
        return fp8_gemm(x, s, weight, weight.scale, scale_dtype, backend="torch")
    else:
...
class Linear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = False, dtype = None):
        ...
        if dtype == torch.float8_e4m3fn:
            self.weight = nn.Parameter(torch.rand(out_features, in_features, dtype=dtype))
...
def hadamard_transform(x: torch.Tensor) -> torch.Tensor:
    ...
    return (x.to(torch.float32) @ h).to(dtype)


def rotate_activation(x: torch.Tensor) -> torch.Tensor:
    """Applies Hadamard rotation to spread information across dims."""
...
class Compressor(nn.Module):
    ...
    def __init__(self, args: ModelArgs, compress_ratio: int = 4, head_dim: int = 512, rotate: bool = False):
        ...
        self.ape = nn.Parameter(torch.rand(compress_ratio, coff * self.head_dim, dtype=torch.float32))
    ...
    def forward(self, x: torch.Tensor, start_pos: int):
        ...
        if self.rotate:
            ...
            # NPU path: skip FP4 fake quant after rotation.
            # fp4_act_quant(kv, fp4_block_size, True)
        else:
            # NPU path: INT8 dynamic quant-dequant for non-RoPE KV dims.
            act_quant(kv[..., :-rd], 64, scale_fmt, scale_dtype, inplace=True)
        if start_pos == 0:
...
class Attention(nn.Module):
    ...
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        self.attn_sink = nn.Parameter(torch.rand(self.n_local_heads, dtype=torch.float32))
...
class Gate(nn.Module):
    ...
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        self.weight = nn.Parameter(torch.rand(args.n_routed_experts, args.dim))
        if self.hash:
            self.tid2eid = nn.Parameter(torch.empty(args.vocab_size, args.n_activated_experts, dtype=torch.int32), requires_grad=False)
            self.bias = None
        else:
            self.bias = nn.Parameter(torch.rand(args.n_routed_experts, dtype=torch.float32))
...
class Block(nn.Module):
    ...
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        with set_dtype(torch.float32):
            self.hc_attn_fn = nn.Parameter(torch.rand(mix_hc, hc_dim))
            self.hc_ffn_fn = nn.Parameter(torch.rand(mix_hc, hc_dim))
            self.hc_attn_base = nn.Parameter(torch.rand(mix_hc))
            self.hc_ffn_base = nn.Parameter(torch.rand(mix_hc))
            self.hc_attn_scale = nn.Parameter(torch.rand(3))
            self.hc_ffn_scale = nn.Parameter(torch.rand(3))
...
class ParallelHead(nn.Module):
    ...
    def __init__(self, vocab_size: int, dim: int, norm_eps: float = 1e-6, hc_eps: float = 1e-6):
        ...
        self.weight = nn.Parameter(torch.rand(self.part_vocab_size, self.dim, dtype=torch.float32))
...
class MTPBlock(Block):
    ...
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        hc_dim = hc_mult * args.dim
        with set_dtype(torch.float32):
            self.hc_head_fn = nn.Parameter(torch.rand(hc_mult, hc_dim))
            self.hc_head_base = nn.Parameter(torch.rand(hc_mult))
            self.hc_head_scale = nn.Parameter(torch.rand(1))
