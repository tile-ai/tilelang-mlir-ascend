...
void CodeGenTileLangNPUIRDEVA5::EmitCopyMemrefToTensor(
  ...
  // 4) Create ub_view matching copy rank, and compute full_sizes for later
  //    tail block detection.
  mlir::Value ub_view;
  auto ubTy = base_ub.getType().cast<mlir::MemRefType>();
  llvm::SmallVector<mlir::OpFoldResult> full_sizes(ubTy.getRank(),
    ...
    if ((int64_t)dstC.keptIdx.size() == (int64_t)copy_sizes.size() &&
      ...
      for (unsigned k = 0; k < dstC.keptIdx.size(); ++k) {
        ...
        if (idx < (unsigned)ubTy.getRank()) {
          full_sizes[idx] = copy_sizes[k];
        }
      }
    } else {
      for (unsigned k = 0;
           k < copy_sizes.size() && k < (unsigned)ubTy.getRank(); ++k) {
        full_sizes[k] = copy_sizes[k];
      }
    }

    ub_view = CreateRankReducedSubviewFromBaseRank(
        base_ub, fullOffsets, full_sizes, fullStrides, copy_projected, loc);
  }

  // 5) Tail block pre-zero: if any dimension's copy size < full alloc size,
  //    zero-fill the entire base_ub.
  ...
  for (int64_t dim_idx = 0; dim_idx < ubTy.getRank(); ++dim_idx) {
    ...
    mlir::Value full_size_val =
        builder.create<mlir::arith::ConstantIndexOp>(loc, static_full_size);
  ...
  auto pad_if_op =
      builder.create<mlir::scf::IfOp>(loc, need_pad, /*addElseRegion=*/false);
  ...
  // 9) Insert slice with optional cast
