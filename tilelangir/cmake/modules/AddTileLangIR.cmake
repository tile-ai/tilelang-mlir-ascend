...
function(_tilelangir_tablegen_one td_rel tblgen_exe root_abs tblgen_args tblgen_includes current_outputs all_outputs_var out_suffix)
  ...
  if(NOT out_suffix)
    set(out_suffix ".h.inc")
  endif()
  string(REGEX REPLACE "\\.td$" "${out_suffix}" out_rel "${td_rel}")
...
function(tilelangir_tablegen target_name)
  ...
  cmake_parse_arguments(ARG "" "OUT_SUFFIX" "TD_FILES;ARGS;EXTRA_INCLUDES;DEPENDS" ${ARGN})
  if(NOT ARG_TD_FILES)
  ...
  foreach(td_rel ${ARG_TD_FILES})
    _tilelangir_tablegen_one("${td_rel}" "${TBLGEN_EXE}" "${BISHENGIR_ROOT_ABS}" "${ARG_ARGS}" "${tblgen_includes}" "${all_outputs}" all_outputs "${ARG_OUT_SUFFIX}")
  endforeach()
