...
def _ssd_chunk_scan_fwd_kernel(
    ...
    def _builder(block_l, block_p, block_n, block_s, num_stages):
        ...
        ws_c_shape = [NUM_KERNELS, P_tiles, L_tiles, bl, N]
        ws_lcb_shape = [NUM_KERNELS, P_tiles, L_tiles, L_tiles, bl, bs]
        ...
        def main(
            ...
            with T.Kernel(NUM_KERNELS, is_npu=True) as (kernel_id, subid):
                ...
                with T.Scope("Cube"):
                    ...
                    l1_x = T.alloc_L1([bs, bp], dtype)
                    l1_lcb = T.alloc_L1([bl, bs], dtype)
                    ...
                    for task_id in T.serial(num_local_tasks):
                        ...
                        with T.rs("PIPE_MTE2"):
                            T.sync_block_wait(0)  # factors ready (Vector -> Cube)
                        ...
                        for pp in T.serial(P_tiles):
                            ...
                            for lt in T.serial(L_tiles):
                                ...
                                for n_blk in T.serial(N_tiles):
                                    ...
                                    tn = T.min(bn, N - n0)
                                    T.copy(
                                        ws_c[kernel_id, pp, lt, 0:tl, n0 : n0 + tn],
                                        l1_c[0:tl, 0:tn],
                                    )
                                    T.copy(
                                ...
                                # ---- intra (full-lower s-blocks + diag) ----
                                # s_blk in [0, lt) is full-lower (no penalty),
                                # s_blk == lt is the diagonal (penalty applied by Vector)
                                for s_blk in T.serial(lt + 1):
                                    s0 = s_blk * bs
                                    ts = T.min(bs, Q - s0)
                                    T.copy(ws_lcb[kernel_id, pp, lt, s_blk, 0:bl, 0:bs], l1_lcb)
                                    T.copy(
                                        x[bz, cs + s0 : cs + s0 + ts, bh, p0 : p0 + tp],
                                        l1_x[0:ts, 0:tp],
                                    )
                                    T.gemm(l1_lcb, l1_x, l0_acc, initC=False, size=[tmc, ts, tnp])
                        ...
                        with T.rs("PIPE_FIX"):
                            T.sync_block_set(1)  # consumed (Cube -> Vector)
                    ...
                    for task_id in T.serial(num_local_tasks):
                        ...
                        for pp in T.serial(P_tiles):
                            ...
                            for lt in T.serial(L_tiles):
                                ...
                                T.copy(c_scaled_16[0:tl, 0:N], ws_c[kernel_id, pp, lt, 0:tl, 0:N])
                                ...
                                for s_blk in T.serial(lt + 1):
                                    ...
                                    T.copy(
                                        lcb_16[0:tl, 0:bs],
                                        ws_lcb[kernel_id, pp, lt, s_blk, 0:tl, 0:bs],
                                    )

                        with T.rs("PIPE_MTE3"):
                            T.sync_block_set(0)  # factors ready (Vector -> Cube)
                        with T.rs("PIPE_MTE2"):
                            T.sync_block_wait(1)  # wait consumed (Cube -> Vector)
    ...
    def _inner(block_l, block_p, block_n, block_s, num_stages):
        ...
        def wrapped(x, cb, dA_cumsum, C_mat, prev_states, dt):
            ws_c = torch.empty(
                (NUM_KERNELS, P_tiles_inner, L_tiles_inner, bl, N),
                dtype=torch_dtype,
                device=x.device,
            )
            ws_lcb = torch.empty(
                (NUM_KERNELS, P_tiles_inner, L_tiles_inner, L_tiles_inner, bl, bs),
                dtype=torch_dtype,
                device=x.device,
            )
...
def _run_case(B, C, Q, H, P, N, G, dtype, tag, seed=0, decay_scale=1.0):
    ...
    wrapped = fn(64, 64, min(64, N), 64, 2)
