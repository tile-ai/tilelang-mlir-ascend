"""Shared reduction primitives for reduction kernels (NPU-adapted).
...
def tune_by_forward(kernel, *probe_inputs, warmup: int = 10, rep: int = 10) -> None:
    ...
    NPU adaptation: uses the device backend abstraction for
    synchronization and timing instead of ``torch.cuda.*`` directly.
