...
class BlockInfo:
    ...
    def is_elementwise(self, sch: tir.Schedule) -> bool:
        ...
        if len(r_region) != len(w_region):
            return False
        for var, r_dom, w_dom in zip(block.iter_vars, r_region, w_region, strict=False):
...
def normalize_prim_func(sch: tir.Schedule) -> Optional[List[BlockInfo]]:
    ...
    for block, loops, iters, is_reduction in zip(*result, strict=False):
        blocks.append(
            BlockInfo(
                ...
                iters=[
                    ...
                    for loop, iter in zip(loops, iters, strict=False)
                ],
                block_rv=block,
