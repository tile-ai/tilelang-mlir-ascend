# tilelang.language.set_flag

## 1. 概述

简介： `tilelang.language.set_flag` 用于在同一个Cube/Vector核内的不同通道之间显式设置同步信号，
调用该接口可以解除对应的 `tilelang.language.wait_flag` 所设置的通道阻塞信号

```python
with T.Scope(this):
    T.set_flag(other, event_id)
```
