...
def test_multi_version_buffer():
    ...
    def before(A: T.Tensor((M, K), dtype), B: T.Tensor((K, N), dtype)):
        ...
        with T.block(""):
            ...
            for i in T.unroll(
                16, annotations={"pragma_unroll_explicit": T.bool(False)}
            ):
            ...
            for k in T.serial(16, annotations={"num_stages": 3}):
                if v == 0:
                    T.tma_load(
                        ...
                        0,
                        T.tvm_access_ptr(
                            T.type_annotation("float16"), A_shared.data, 0, 2048, 2
                        ),
                        k * 32,
                        by * 64,
                    )
                if v == 0:
                    T.tma_load(
                        ...
                        0,
                        T.tvm_access_ptr(
                            T.type_annotation("float16"), B_shared.data, 0, 2048, 2
                        ),
                        bx * 64,
                        k * 32,
                    )
                T.call_extern(
                    ...
                    T.tvm_access_ptr(
                        T.type_annotation("float32"), C_local.data, 0, 32, 3
                    ),
                )
