...
class KernelCache:
    """
    Caches compiled NPU kernels using a class and database persistence to avoid
    ...
    def cached_npu(
        ...
        1. In-memory cache  — cheapest, per-process.
    ...
    def load_compile_result(
        ...
        Returns ``None`` on cache miss (metadata absent or any required
        artefact missing/corrupt).
        """
        cache_path = self._get_cache_path(key)
    ...
    def save_autotune_result(
        ...
        NPU kernel artefacts are written at the entry root.  The three tuning
        metadata files go into the ``autotune/`` subfolder so they are easy
        to spot and never collide with kernel files.
        """
        cache_path = self._get_cache_path(key)
