            ...
            for n in T.Pipelined(T.ceildiv(num_heads, block_heads), num_stages=2):
                ...
                for k in T.Pipelined(T.ceildiv(top_k, block_top_k), num_stages=2):
                    ...
                    if n == 0:
                        ...
                        for i in T.serial(real_block_top_k):
                            ...
                            if cur_idx != -1:
                                valid_mask[0, i] = 1.0
                                T.copy(
                                    KV[by, cur_idx, 0], kv_shared[i, 0], size=[1, dim]
                                )
                    ...
                    for i in T.Parallel(block_heads):
                        scores_scale[i, 0] = T.exp(
                            scores_max_prev[i, 0] - scores_max[i, 0]
                        )
                    for i, j in T.Parallel(block_heads, block_top_k):
                    ...
                    for i in T.Parallel(block_heads):
                        sum_exp[i, 0] = (
                            sum_exp[i, 0] * scores_scale[i, 0] + scores_sum[i, 0]
                        )
...
def sparse_attn(
    ...
    sparse_kv_buffer = torch.empty(
        (batch_size, seq_len, next_divisible_number(top_k, block), dim),
        dtype=q.dtype,
        device=q.device,
    )
    valid_mask_buffer = torch.empty(
        (batch_size, seq_len, next_divisible_number(top_k, block)),
