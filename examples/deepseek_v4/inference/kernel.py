...
FP32 = "float32"
INT32 = "int32"
DEFAULT_KERNEL_BACKEND = "torch"


def _resolve_backend(backend: Optional[str]) -> str:
    return DEFAULT_KERNEL_BACKEND if backend is None else backend


def _round_scale_to_pow2(scale: torch.Tensor) -> torch.Tensor:
    return torch.pow(torch.full_like(scale, 2.0), torch.ceil(torch.log2(scale)))
...
def _act_quant_torch(
    ...
    """Torch implementation of act_quant_kernel.

    This mirrors the TileLang path for correctness/debugging: block-wise scale
    computation, FP8 rounding, optional dequantize-back for inplace simulation.
    """
    N = x.size(-1)
    assert N % block_size == 0
    z = x.contiguous()
    original_shape = z.shape
    x_blocks = z.float().view(-1, N // block_size, block_size)
    scale = x_blocks.abs().amax(dim=-1).clamp_min(1e-4) * (1 / 448.0)
    if scale_fmt is not None:
        scale = _round_scale_to_pow2(scale)

    q = torch.clamp(x_blocks / scale.unsqueeze(-1), -448.0, 448.0).to(torch.float8_e4m3fn)
    s = scale.reshape(*original_shape[:-1], N // block_size).to(scale_dtype)
    if inplace:
        y = (q.float() * scale.unsqueeze(-1)).to(z.dtype).reshape(original_shape)
        x.copy_(y)
        return x
    return q.reshape(original_shape), s
...
def act_quant(
    ...
    """NPU dynamic quantization.

    For inplace fake quant, quantize to NPU-supported INT8 and dequantize back
    to the input dtype. The unused args stay in the signature for call-site
    compatibility with the original FP8 API.
    """
    z = x.contiguous()
    y, s = torch.ops.npu.npu_dynamic_quant(z)
    if inplace:
        x.copy_((y.float() * s.unsqueeze(-1)).to(x.dtype))
        return x
    return y, s
...
def _fp8_gemm_torch(
    ...
    """Torch implementation of fp8_gemm_kernel.

    Dequantizes A and B with the same per-block scales used by the TileLang
    kernel, then computes C = A @ B.T in torch.
    """
    ...
    a_flat = a.view(-1, K)
    a_scale = a_s.float().view(-1, a_s.size(-1)).repeat_interleave(128, dim=-1)
    a_dequant = a_flat.float() * a_scale[:, :K]

    b_scale = b_s.float().repeat_interleave(128, dim=0).repeat_interleave(128, dim=1)
    b_dequant = b.float() * b_scale[:N, :K]

    c = a_dequant @ b_dequant.t()
    return c.reshape(*original_shape[:-1], N).to(torch.get_default_dtype())


def fp8_gemm(
    a: torch.Tensor, a_s: torch.Tensor, b: torch.Tensor, b_s: torch.Tensor,
    scale_dtype: torch.dtype = torch.float32, backend: Optional[str] = None,
) -> torch.Tensor:
    ...
    if backend != "tilelang":
        raise ValueError(f"Unsupported fp8_gemm backend: {backend}")
    assert a.is_contiguous() and b.is_contiguous(), "Input tensors must be contiguous"
    assert a_s.is_contiguous() and b_s.is_contiguous(), (
        "Scaling factor tensors must be contiguous"
    )
